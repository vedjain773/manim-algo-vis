...
class Main(Scene):
    def construct(self):
        ...
        def highlightAB(obj1: VGroup, obj2: VGroup):
            self.play(obj1[0].animate.set_fill(BLUE, opacity=0.7))
            self.play(obj2[0].animate.set_fill(BLUE_A, opacity=0.7))
        ...
        while (isSorted(array) == False):
            for i in range(len(array) - 1):
                highlightAB(mObjArr[i], mObjArr[i+1])

                if array[i] > array[i + 1]:
                    swap(i, i + 1, array)
                    highlightAB(mObjArr[i], mObjArr[i+1])
                    highlightGR(mObjArr[i], mObjArr[i+1])

                    switchPosition(mObjArr[i], mObjArr[i+1])
                    deHighlight(mObjArr[i], mObjArr[i+1])
                    swap(i, i + 1, mObjArr)
                else:
                    highlightAB(mObjArr[i], mObjArr[i+1])
